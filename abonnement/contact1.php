...
if ($action == 'addcontact' && $user->rights->contract->write) {
	if ($object->id > 0) {
...
else if ($action == 'swapstatut' && $user->rights->contrat->write) {
	if ($object->id > 0) {
...
// Efface un contact
else if ($action == 'deletecontact' && $user->rights->contrat->write) {
...
if ($object->id > 0) {
	...
	dol_fiche_head($head, 'contact', $langs->trans("Contact"), 0, 'contact');
